...
const schema = readFileSync('packages/durability/src/durable-schema.sql', 'utf8');

const baseRecord = (): AuditRecord => ({
  auditId: 'audit-1', tenantId: 'tenant-a', occurredAt: '2026-09-14T10:00:00.000Z', actorId: 'user-1', actorType: 'USER',
  action: 'PAUSE', resourceType: 'RUN', resourceId: 'run-1', outcome: 'SUCCEEDED', reasonClass: 'NONE',
  correlation: { requestId: 'req-1', traceId: 'trace-1' }, metadata: {},
});
...
describe('auditability immutability and sensitive-data boundary', () => {
  ...
  it('exposes an append/query-only in-memory audit boundary', async () => {
    ...
    const result = await repository.query({ tenantId: 'tenant-a', from: '2026-09-14T00:00:00.000Z', to: '2026-09-15T00:00:00.000Z', limit: 100 });
