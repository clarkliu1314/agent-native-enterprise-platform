...
describe('auditability production composition RED gate', () => {
  ...
  it('serves the audit query through the production Vercel handler boundary', async () => {
    ...
    await audit.append({
      auditId: 'audit-production-1', tenantId: 'tenant-a', occurredAt: '2026-09-14T12:00:00.000Z',
      actorId: 'user-1', actorType: 'USER', action: 'PAUSE', resourceType: 'RUN', resourceId: 'run-1',
      outcome: 'SUCCEEDED', reasonClass: 'NONE', correlation: { requestId: 'req-1', traceId: 'trace-1' },
      version: 1, metadata: { resultingState: 'PAUSED' },
    });

    const auditQueryHandler = (await import('../../../apps/api/src/audit-query-handler')).createAuditQueryHandler({
    ...
    await expect(response.json()).resolves.toMatchObject({ items: [{ auditId: 'audit-production-1', tenantId: 'tenant-a', actorId: 'user-1' }] });
  });
  ...
  it('rejects an unauthorized actor without querying audit data', async () => {
    ...
    const service = new AuditQueryService({
      query: async () => { queried = true; return { items: [] }; },
