...
describe('investment domain durable E2E', () => {
  it('persists one decision, one business event, and one outbox effect for duplicate approval', async () => {
    ...
    try {
      ...
      const first = await service.approve({ tenantId, opportunityId, actorId, decisionCycle: 1, recommendation: 'APPROVE', rationale: 'E2E approval', idempotencyKey: decisionKey });
      const second = await service.approve({ tenantId, opportunityId, actorId, decisionCycle: 1, recommendation: 'APPROVE', rationale: 'E2E approval', idempotencyKey: decisionKey });
      ...
      const opportunity = await pool.query<{ stage: string; status: string }>('SELECT stage, status FROM investment_opportunities WHERE tenant_id = $1 AND opportunity_id = $2', [tenantId, opportunityId]);
