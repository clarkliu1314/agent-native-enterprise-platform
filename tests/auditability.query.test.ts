...
const record = (auditId: string, tenantId: string): AuditRecord => ({
  auditId, tenantId, occurredAt: '2026-09-14T10:00:00.000Z', actorId: 'user-1', actorType: 'USER',
  action: 'PAUSE', resourceType: 'RUN', resourceId: 'run-1', outcome: 'SUCCEEDED', reasonClass: 'NONE',
  correlation: { requestId: `req-${auditId}`, traceId: `trace-${auditId}` }, metadata: {},
});
...
describe('audit query application/API boundary', () => {
  it('queries only the authenticated tenant and enforces bounded query parameters', async () => {
    ...
    await repository.append(record('audit-a', 'tenant-a'));
    await repository.append(record('audit-b', 'tenant-b'));
  ...
  it('exposes tenant and actor from headers, never from the query string or request body', async () => {
    ...
    await expect(response.json()).resolves.toMatchObject({ items: [{ auditId: 'audit-a', tenantId: 'tenant-a' }] });
  });
});
