...
describe('audit query application/API boundary', () => {
  it('queries only the authenticated tenant and enforces bounded query parameters', async () => {
    ...
    await expect(service.query({ actorId: 'user-1', tenantId: 'tenant-a', from: '2026-09-01T00:00:00.000Z', to: '2026-10-03T00:00:00.000Z', limit: 100 })).rejects.toThrow('INVALID_AUDIT_QUERY_WINDOW');
    await expect(service.query({ actorId: 'user-1', tenantId: 'tenant-a', from: '2026-09-14T00:00:00.000Z', to: '2026-09-15T00:00:00.000Z', limit: 101 })).rejects.toThrow('INVALID_AUDIT_QUERY_LIMIT');
  });
