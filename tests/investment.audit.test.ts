...
import type { InvestmentSqlClient } from '../packages/investment-domain/src/application-service';
...
describe('investment auditability', () => {
  it('audits a material investment approval once at the same transaction boundary as the domain event', async () => {
    ...
    expect(audits.items[0]).toMatchObject({
      actorId: 'user-1', actorType: 'USER', action: 'INVESTMENT_APPROVED',
      resourceType: 'INVESTMENT_OPPORTUNITY', resourceId: 'opp-1', outcome: 'SUCCEEDED',
      metadata: { decisionCycle: 1, resultingStage: 'APPROVED' },
    });
  });
});
