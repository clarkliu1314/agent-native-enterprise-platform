...
describe('material runtime audit', () => {
  it('audits RUN_CREATED inside the same durable transaction', async () => {
    const auditRecords: AuditRecord[] = [];
    let transactionClient: TransactionClient | undefined;
    let auditTransactionClient: TransactionClient | undefined;
    const audit: AuditRepository & { appendInTransaction?: (tx: TransactionClient, record: AuditRecord) => Promise<void> } = {
      ...
      appendInTransaction: async (tx, record) => { auditTransactionClient = tx; auditRecords.push(record); },
      query: async () => ({ items: [] }),
    };
    const runRow = {
      ...
      }, fencing_token: 0n, attempt: 0, version: 0, created_at: '2026-09-14T10:00:00.000Z',
    };
    const eventRow = {
    ...
    const db = {
      query: async <T extends Record<string, unknown>>(): Promise<SqlResult<T>> => result<T>(),
    ...
    expect(auditTransactionClient).toBe(transactionClient);
