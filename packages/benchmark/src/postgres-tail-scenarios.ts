...
export async function executePostgresTailScenario(
  ...
  try {
    ...
    if (caseId === 'B14') {
      ...
      const firstFailure = await recoveryStore.recordRecoveryFailure({
        runId,
        owner: firstLease.owner,
        leaseToken: firstLease.leaseToken,
        retryable: true,
        error: new Error('retryable failure 1'),
        now: firstNow,
        baseBackoffMs: 1_000,
        maxBackoffMs: 2_000,
      });
      ...
      const secondFailure = await recoveryStore.recordRecoveryFailure({
        runId,
        owner: secondLease.owner,
        leaseToken: secondLease.leaseToken,
        retryable: true,
        error: new Error('retryable failure 2'),
        now: secondNow,
        baseBackoffMs: 1_000,
        maxBackoffMs: 2_000,
      });
    ...
    if (caseId === 'B15') {
      ...
      const failure = await recoveryStore.recordRecoveryFailure({
        runId,
        owner: lease.owner,
    ...
    const service = new ToolExecutionService({
      ...
      execute: async () => {
        ...
        return { caseId, adapter, recovered: true };
      },
      store: toolStore,
    });
...
async function seedCandidate(pool: Pool, runId: string, request: ToolExecutionRequest): Promise<void> {
  await pool.query(
    `INSERT INTO agent_runs (run_id, agent_id, state, input, version, metadata, recovery_state, recovery_attempts, next_attempt_at)
     VALUES ($1, 'benchmark-agent', 'RUNNING', '{}', 0, $2::jsonb, 'IN_PROGRESS', 0, NOW())`,
    [runId, JSON.stringify({ recovery_request: request })],
  );
}
...
async function cleanup(pool: Pool, prefix: string, runId: string): Promise<void> {
...
function result(caseId: BenchmarkCase['id'], adapter: BenchmarkAdapter, violations: string[], details: string): BenchmarkScenarioResult {
  return { invariantViolations: violations, details: `case: ${caseId}; ${details}` };
}
