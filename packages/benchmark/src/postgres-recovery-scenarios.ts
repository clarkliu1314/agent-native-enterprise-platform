...
export async function executePostgresRecoveryScenario(caseId: BenchmarkCase['id'], adapter: BenchmarkAdapter, databaseUrl: string): Promise<BenchmarkScenarioResult> {
  ...
  try {
    ...
    if (caseId === 'B14') {
      ...
      const attempts = Number(row.rows[0]?.recovery_attempts ?? -1);
      const terminalState = row.rows[0]?.recovery_state;
    ...
    if (caseId === 'B16') {
      ...
      if (winners[0]) {
        recovered = await coordinator.recover({ request, state: 'IN_PROGRESS' });
      ...
      const terminalCompletions = row.rows[0]?.recovery_state === 'SUCCEEDED' ? 1 : 0;
      const claimWinners = winners.length;
