...
import { investmentBenchmarkCases } from './index';

describe('investment durability benchmark', () => {
  it('defines the four investment durability cases', () => {
    expect(investmentBenchmarkCases.map((testCase) => testCase.id)).toEqual(['B17', 'B18', 'B19', 'B20']);
  });
  ...
  it('uses the shared benchmark contract and adapter matrix', () => {
    for (const testCase of investmentBenchmarkCases) {
      ...
      expect(testCase.invariants).toEqual(expect.arrayContaining(['permission', 'idempotency', 'outbox']));
