...
import {
  buildSensitiveDataBoundary,
  sanitizeBoundaryPayload,
  executeDuplicateRequestProbe,
  type DuplicateRequestProbeInput,
} from './security-concurrency-readiness.js';
...
describe('stage 12.6 Task 5 readiness contracts', () => {
  it('redacts sensitive fields at the data boundary without changing safe fields', () => {
    ...
    const input = {
      ...
      requestId: 'req-1',
      password: 'pw-123',
      nested: { apiKey: 'key-456', label: 'safe' },
  ...
  it('fails closed for sensitive values inside arrays and deeply nested objects', () => {
    ...
    const sanitized = sanitizeBoundaryPayload(boundary, {
      items: [{ token: 'abc' }, { nested: { privateKey: 'pem' } }],
      status: 'ok',
    });

    expect(sanitized).toEqual({
      items: [{ token: '[REDACTED]' }, { nested: { privateKey: '[REDACTED]' } }],
      status: 'ok',
    });
  });

  it('accepts only the first request for the same idempotency key', async () => {
    const input: DuplicateRequestProbeInput = {
  ...
  it('does not allow a failed first attempt to be silently replayed as success', async () => {
    let calls = 0;
    const result = await executeDuplicateRequestProbe({
