...
export interface OperationalControlEvent { eventId: string; runId: string; type: 'OPERATIONAL_CONTROL_APPLIED'; payload: { commandId: string; tenantId: string; runId: string; actorId: string; action: OperationalControlAction; outcome: OperationalControlOutcome; reasonClass: 'NONE' | 'PROVIDED'; correlation: Pick<CorrelationContext, 'requestId' | 'traceId' | 'tenantId' | 'runId' | 'workflowId' | 'agentId' | 'actorId'>; resultingState: Pick<OperationalControlState, 'paused' | 'cancelled' | 'runState' | 'version'>; }; }
