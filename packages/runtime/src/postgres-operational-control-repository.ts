...
export class PostgresOperationalControlRepository implements OperationalControlRepository {
  ...
  async apply(command: OperationalControlCommand, _next: OperationalControlState, event: OperationalControlEvent): Promise<OperationalControlResult> {
    return this.db.transaction(async (tx) => {
      ...
      const inserted = await tx.query('INSERT INTO agent_events (event_id,run_id,sequence,type,payload) VALUES ($1,$2,$3,$4,$5::jsonb)', [eventId, command.runId, seq.toString(), durableEvent.type, json(durableEvent.payload)]); if (inserted.rowCount !== 1) throw new Error('Operational control event write rejected');
...
function applyAction(action: OperationalControlAction, current: OperationalControlState): OperationalControlState {
  if (action === 'PAUSE') return { ...current, paused: true, version: current.version + 1 };
  if (action === 'RESUME') return { ...current, paused: false, version: current.version + 1 };
  if (action === 'CANCEL') return { ...current, cancelled: true, paused: false, runState: 'CANCELLED', version: current.version + 1, fencingToken: current.fencingToken + 1n };
  return current;
}
