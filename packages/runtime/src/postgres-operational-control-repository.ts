...
export class PostgresOperationalControlRepository implements OperationalControlRepository {
  ...
  async get(tenantId: string, runId: string): Promise<OperationalControlState | null> {
    const result = await this.db.query<Record<string, unknown>>('SELECT run_id,state,version,metadata,fencing_token FROM agent_runs WHERE run_id=$1 AND metadata->>\'tenantId\'=$2', [runId, tenantId]);
    return result.rows[0] ? toState(result.rows[0], tenantId) : null;
  }

  async findByIdempotency(tenantId: string, key: string): Promise<OperationalControlResult | null> {
    const result = await this.db.query<Record<string, unknown>>('SELECT response FROM idempotency_keys WHERE idempotency_key=$1', [controlKey(tenantId, key)]);
    return result.rows[0]?.response ? deserializeResult(result.rows[0].response) : null;
  }
  ...
  async apply(command: OperationalControlCommand, next: OperationalControlState, event: OperationalControlEvent): Promise<OperationalControlResult> {
    return this.db.transaction(async (tx) => {
      ...
      if (!row || String((row.metadata as Record<string, unknown> | undefined)?.tenantId ?? '') !== command.tenantId) throw new Error('Operational control target not found');
      const current = toState(row, command.tenantId);
      if (command.expectedVersion !== undefined && current.version !== command.expectedVersion) throw new Error('Operational control version is stale');
      ...
      const inserted = await tx.query('INSERT INTO agent_events (event_id,run_id,sequence,type,payload) VALUES ($1,$2,$3,$4,$5::jsonb)', [eventId, command.runId, seq.toString(), event.type, json(event.payload)]);
      if (inserted.rowCount !== 1) throw new Error('Operational control event write rejected');
  ...
  async listEvents(runId: string): Promise<OperationalControlEvent[]> {
    const result = await this.db.query<Record<string, unknown>>('SELECT event_id,run_id,type,payload FROM agent_events WHERE run_id=$1 AND type=\'OPERATIONAL_CONTROL_APPLIED\' ORDER BY sequence', [runId]);
    return result.rows.map((row) => ({ eventId: String(row.event_id), runId: String(row.run_id), type: 'OPERATIONAL_CONTROL_APPLIED', payload: row.payload as OperationalControlEvent['payload'] }));
  }
}
...
function toState(row: Record<string, unknown>, tenantId: string): OperationalControlState {
  return { tenantId, runId: String(row.run_id), version: Number(row.version), paused: Boolean((currentMetadata(row).operationalControl as Record<string, unknown> | undefined)?.paused), cancelled: Boolean((currentMetadata(row).operationalControl as Record<string, unknown> | undefined)?.cancelled), runState: row.state as OperationalControlState['runState'], fencingToken: BigInt(String(row.fencing_token)) };
}
function deserializeResult(value: unknown): OperationalControlResult { const result = value as OperationalControlResult; return { ...result, control: { ...result.control, fencingToken: BigInt(String((result.control as OperationalControlState).fencingToken)) } }; }
