...
import type { AuditRecord } from './auditability';
import { insertAuditRecord } from './postgres-audit-repository';
import type { SqlClient, TransactionRunner } from './ports';
import { type OperationalControlAction, type OperationalControlCommand, type OperationalControlEvent, type OperationalControlRepository, type OperationalControlResult, type OperationalControlState } from './operational-control';
...
export class PostgresOperationalControlRepository implements OperationalControlRepository {
  ...
  async apply(command: OperationalControlCommand, _next: OperationalControlState, event: OperationalControlEvent, audit?: AuditRecord): Promise<OperationalControlResult> {
    return this.db.transaction(async (tx) => {
      ...
      const row = locked.rows[0]; const metadata = row?.metadata as Record<string, unknown> | undefined; const rowTenantId = String(metadata?.tenantId ?? '');
      if (!row || rowTenantId !== command.tenantId) throw new Error('Operational control target not found');
      ...
      const durableEvent: OperationalControlEvent = { ...event, payload: { ...event.payload, resultingState: { paused: next.paused, cancelled: next.cancelled, runState: next.runState, version: next.version } } };
      const seq = await nextSequence(tx, command.runId); const eventId = durableEvent.eventId;
      const inserted = await tx.query('INSERT INTO agent_events (event_id,run_id,sequence,type,payload) VALUES ($1,$2,$3,$4,$5::jsonb)', [eventId, command.runId, seq.toString(), durableEvent.type, json(durableEvent.payload)]);
      if (inserted.rowCount !== 1) throw new Error('Operational control event write rejected');
      if (audit) await insertAuditRecord(tx, audit);
      await tx.query('INSERT INTO outbox_events (outbox_id,event_id,topic,payload,idempotency_key,tenant_id,actor_id,event_type) VALUES ($1,$2,$3,$4::jsonb,$5,$6,$7,$8)', [`outbox:${eventId}`, eventId, 'agent.run', json({ eventId, runId: command.runId, sequence: seq.toString(), type: durableEvent.type, payload: durableEvent.payload }), command.idempotencyKey, command.tenantId, command.actorId, durableEvent.type]);
...
function applyAction(action: OperationalControlAction, current: OperationalControlState): OperationalControlState { if (action === 'PAUSE') return { ...current, paused: true, version: current.version + 1 }; if (action === 'RESUME') return { ...current, paused: false, version: current.version + 1 }; if (action === 'CANCEL') return { ...current, cancelled: true, paused: false, runState: 'CANCELLED', version: current.version + 1, fencingToken: current.fencingToken + 1n }; return current; }
