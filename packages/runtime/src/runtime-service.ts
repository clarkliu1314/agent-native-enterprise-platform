...
export interface RuntimeServiceOptions { leaseMs?: number; heartbeatMs?: number; clock?: Clock; ids?: IdGenerator; adapter: RuntimeAdapter; }
export interface ClaimedRunExecution { runId: string; owner: string; fencingToken: bigint; }
...
export class DurableRuntimeService implements RuntimeFacade {
  ...
  async executeClaimedRunBounded(claim: ClaimedRunExecution, deadlineAt: Date): Promise<ExecuteBoundedResult> {
    ...
    if (run.state !== 'RUNNING' || run.fencingToken !== claim.fencingToken) return { run, terminal: run.state === 'SUCCEEDED' || run.state === 'FAILED' || run.state === 'CANCELLED' };
    ...
    ).start({ ...claim, leaseMs: this.leaseMs }, () => executionController.abort(new DOMException('Run lease lost', 'AbortError')));
    try {
      ...
      if (isDeadlineError(error)) { const current = await this.requireRun(claim.runId); return { run: current, terminal: current.state === 'SUCCEEDED' || current.state === 'FAILED' || current.state === 'CANCELLED' }; }
      throw error;
    } finally { heartbeat.stop(); clearTimeout(timeout); }
  }
