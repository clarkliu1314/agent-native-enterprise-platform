...
export class DurableRuntimeService implements RuntimeFacade {
  ...
  async createRun(command: CreateRunCommand) {
    ...
    if (key) {
      ...
      if (existing) { if (existing.commandHash !== commandHash) throw new IdempotencyConflictError(key); if (!existing.runId) throw new Error(`Incomplete idempotency record: ${key}`); return { run: await this.requireRun(existing.runId), replayed: true }; }
    }
    ...
    try { return { run: await this.repos.admitRun({ command, commandHash, runId, eventId }), replayed: false }; }
      ...
      if (!key) throw error;
      const existing = await this.repos.getIdempotency(key);
      if (existing?.commandHash === commandHash && existing.runId) return { run: await this.requireRun(existing.runId), replayed: true };
      throw error;
    }
  }
  async resumeRun(runId: string, owner: string): Promise<RunView> {
    ...
    if (!claimed) throw new Error(`Run cannot be claimed: ${runId}`);
    return claimed.run;
  }
  async cancelRun(runId: string, reason?: string): Promise<RunView> {
    const run = await this.requireRun(runId); if (run.state === 'SUCCEEDED' || run.state === 'FAILED' || run.state === 'CANCELLED') return run;
    ...
    const event = await this.repos.appendEvent({ runId, type: 'RUN_CANCELLED', payload: { reason } }); await this.repos.createOutbox({ eventId: event.eventId, topic: 'agent.run', payload: event.payload }); return updated;
  }
  async approveRun(runId: string, approvalId: string): Promise<RunView> {
    const run = await this.requireRun(runId); if (run.state !== 'WAITING') throw new Error(`Run is not waiting for approval: ${runId}`);
    assertValidDurableTransition('WAITING', 'QUEUED');
    const updated = await this.repos.transitionRun({ runId, from: 'WAITING', to: 'QUEUED' }); const event = await this.repos.appendEvent({ runId, type: 'RUN_APPROVED', payload: { approvalId } }); await this.repos.createOutbox({ eventId: event.eventId, topic: 'agent.run', payload: event.payload }); return updated;
  }
  async executeRunBounded(runId: string, owner: string, deadlineAt: Date): Promise<ExecuteBoundedResult> {
    ...
    if (nextState === 'WAITING') { assertValidDurableTransition('RUNNING', 'WAITING'); run = await this.repos.transitionRun({ runId, owner, fencingToken: run.fencingToken, from: 'RUNNING', to: 'WAITING' }); }
    else if (nextState === 'SUCCEEDED') { assertValidDurableTransition('RUNNING', 'SUCCEEDED'); run = await this.repos.transitionRun({ runId, owner, fencingToken: run.fencingToken, from: 'RUNNING', to: 'SUCCEEDED' }); }
    else { assertValidDurableTransition('RUNNING', 'FAILED'); run = await this.repos.transitionRun({ runId, owner, fencingToken: run.fencingToken, from: 'RUNNING', to: 'FAILED', error: result.error }); }
    const event = await this.repos.appendEvent({ runId, type: `RUN_${result.kind}`, payload: { output: result.output, error: result.error }, fencingToken: run.fencingToken }); await this.repos.createOutbox({ eventId: event.eventId, topic: 'agent.run', payload: event.payload });
