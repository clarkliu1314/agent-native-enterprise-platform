...
export interface RunClaim { run: RunView; fencingToken: bigint; }
export interface IdempotencyRecord { key: string; commandHash: string; runId?: string; response?: unknown; }
export interface ToolCallRecord {
  toolCallId: string; runId: string; fencingToken: bigint; idempotencyKey: string; toolName: string;
  kind: 'PURE' | 'SIDE_EFFECTING'; status: 'REQUESTED' | 'RUNNING' | 'SUCCEEDED' | 'FAILED' | 'WAITING';
...
export interface OutboxRecord { outboxId: string; eventId: string; topic: string; payload: unknown; attempts: number; }
