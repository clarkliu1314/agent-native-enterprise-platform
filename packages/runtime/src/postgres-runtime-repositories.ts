...
export class PostgresRuntimeRepositories implements DurableRepositories {
  ...
  async admitRun(input: { command: CreateRunCommand; commandHash: string; runId: string; eventId: string }): Promise<RunView> {
    return this.db.transaction(async (tx) => {
      ...
      await tx.query(`INSERT INTO outbox_events (outbox_id, event_id, topic, payload) VALUES ($1, $2, 'agent.run', $3::jsonb)`, [`outbox:${input.eventId}`, input.eventId, json(event.rows[0].payload)]);
  ...
  async transitionRunAndEmit(input: { runId: string; fencingToken?: bigint; from: RunView['state']; to: RunView['state']; owner?: string; error?: string; eventType: string; eventPayload: unknown; topic: string }): Promise<RunView> {
    return this.db.transaction(async (tx) => {
      ...
      await tx.query(`INSERT INTO outbox_events (outbox_id,event_id,topic,payload) VALUES ($1,$2,$3,$4::jsonb)`, [`outbox:${event.eventId}`, event.eventId, input.topic, json(event.payload)]);
  ...
  async appendEventAndOutbox(input: { runId: string; type: string; payload: unknown; topic: string; fencingToken?: bigint }): Promise<RuntimeEventView> { return this.db.transaction(async (tx) => { const event = await this.appendEventInTx(tx, input); await tx.query(`INSERT INTO outbox_events (outbox_id,event_id,topic,payload) VALUES ($1,$2,$3,$4::jsonb)`, [`outbox:${event.eventId}`, event.eventId, input.topic, json(event.payload)]); return event; }); }
