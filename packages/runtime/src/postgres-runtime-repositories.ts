...
export class PostgresRuntimeRepositories implements DurableRepositories {
  ...
  async createToolCall(input: any): Promise<void> { throw new Error('Not implemented'); }
  async getToolCall(toolCallId: string): Promise<any> { return null; }
  async completeToolCall(input: any): Promise<boolean> { return false; }
  ...
  async saveRunProgress(input: { runId: string; owner: string; fencingToken: bigint; metadata: Record<string, unknown>; checkpoint: CheckpointEnvelope }): Promise<void> {
    await this.db.transaction(async (tx) => {
      const updated = await tx.query(`UPDATE agent_runs SET metadata=$2::jsonb, version=version+1 WHERE run_id=$1 AND state='RUNNING' AND lease_owner=$3 AND fencing_token=$4::bigint`, [input.runId, json(input.metadata), input.owner, input.fencingToken.toString()]);
      ...
      if (inserted.rowCount !== 1) throw new Error(`Checkpoint persistence rejected for ${input.runId}`);
    });
  }
