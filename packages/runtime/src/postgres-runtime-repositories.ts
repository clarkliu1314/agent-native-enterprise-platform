...
export class PostgresRuntimeRepositories implements DurableRepositories {
  ...
  async saveCheckpoint(checkpoint: CheckpointEnvelope): Promise<void> {
    const result = await this.db.query(`INSERT INTO checkpoints (checkpoint_id,run_id,turn_id,sequence,fencing_token,adapter,adapter_version,schema_version,payload) SELECT $1,run_id,$3,$4,$5,$6,$7,$8,$9 FROM agent_runs WHERE run_id=$2 AND state='RUNNING' AND fencing_token=$5::bigint`, [checkpoint.checkpointId,checkpoint.runId,checkpoint.turnId??null,checkpoint.sequence.toString(),checkpoint.fencingToken.toString(),checkpoint.adapter,checkpoint.adapterVersion,checkpoint.schemaVersion,checkpoint.payload]);
    if (result.rowCount !== 1) throw new Error(`Fenced checkpoint write rejected for ${checkpoint.runId}`);
  }
  async saveRunProgress(input: { runId: string; fencingToken: bigint; metadata: Record<string, unknown>; checkpoint: CheckpointEnvelope }): Promise<void> {
    await this.db.transaction(async (tx) => {
      const updated = await tx.query(`UPDATE agent_runs SET metadata=$2::jsonb, version=version+1 WHERE run_id=$1 AND state='RUNNING' AND fencing_token=$3::bigint`, [input.runId, json(input.metadata), input.fencingToken.toString()]);
      if (updated.rowCount !== 1) throw new Error(`Fenced run progress write rejected for ${input.runId}`);
      const checkpoint = input.checkpoint;
      const inserted = await tx.query(`INSERT INTO checkpoints (checkpoint_id,run_id,turn_id,sequence,fencing_token,adapter,adapter_version,schema_version,payload) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)`, [checkpoint.checkpointId, checkpoint.runId, checkpoint.turnId ?? null, checkpoint.sequence.toString(), checkpoint.fencingToken.toString(), checkpoint.adapter, checkpoint.adapterVersion, checkpoint.schemaVersion, checkpoint.payload]);
      if (inserted.rowCount !== 1) throw new Error(`Checkpoint persistence rejected for ${checkpoint.runId}`);
    });
  }
