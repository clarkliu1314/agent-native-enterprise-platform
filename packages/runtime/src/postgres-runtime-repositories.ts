...
export class PostgresRuntimeRepositories implements DurableRepositories {
  ...
  async admitRun(input: { command: CreateRunCommand; commandHash: string; runId: string; eventId: string }): Promise<RunView> {
    return this.db.transaction(async (tx) => {
      ...
      const event = await tx.query<Record<string, unknown>>(`INSERT INTO agent_events (event_id, run_id, sequence, type, payload) VALUES ($1, $2, 1, 'RUN_CREATED', $3::jsonb) RETURNING *`, [input.eventId, input.runId, json({ executionMode: input.command.executionMode ?? 'async', correlation: { requestId: input.command.metadata?.requestId, traceId: input.command.metadata?.traceId, tenantId: input.command.metadata?.tenantId, runId: input.runId, agentId: input.command.agentId } })]);
      const eventView = toEventView(event.rows[0]);
      const run = toRunView(inserted.rows[0]);
      const audit = buildRunLifecycleAuditRecord({ run, tenantId: String(input.command.metadata?.tenantId ?? ''), version: Number(inserted.rows[0].version ?? 0), action: 'RUN_CREATED', occurredAt: eventView.createdAt, correlation: { requestId: String(input.command.metadata?.requestId ?? eventView.eventId), traceId: String(input.command.metadata?.traceId ?? eventView.eventId) } });
      await this.appendAuditInTransaction(tx, audit);
  ...
  async transitionRunAndEmit(input: { runId: string; fencingToken?: bigint; from: RunView['state']; to: RunView['state']; owner?: string; error?: string; eventType: string; eventPayload: unknown; topic: string }): Promise<RunView> {
    return this.db.transaction(async (tx) => {
      ...
      const audit = buildRunLifecycleAuditRecord({ run, tenantId: String(run.metadata.tenantId ?? ''), version: Number(result.rows[0].version ?? 0), action: input.eventType, occurredAt: event.createdAt, actorId: 'system', correlation: { requestId: String(run.metadata.requestId ?? event.eventId), traceId: String(run.metadata.traceId ?? event.eventId) }, fromState: input.from });
      await this.appendAuditInTransaction(tx, audit);
  ...
  async saveCheckpoint(checkpoint: CheckpointEnvelope): Promise<void> { const result = await this.db.query(`INSERT INTO checkpoints (checkpoint_id,run_id,turn_id,sequence,fencing_token,adapter,adapter_version,schema_version,payload) SELECT $1,run_id,$3,$4,$5,$6,$7,$8,$9 FROM agent_runs WHERE run_id=$2 AND state='RUNNING' AND fencing_token=$5::bigint`, [checkpoint.checkpointId,checkpoint.runId,checkpoint.turnId??null,checkpoint.sequence.toString(),checkpoint.fencingToken.toString(),checkpoint.adapter,checkpoint.adapterVersion,checkpoint.schemaVersion,checkpoint.payload]); if (result.rowCount !== 1) throw new Error(`Fenced checkpoint write rejected for ${checkpoint.runId}`); }
  async saveRunProgress(input: { runId: string; fencingToken: bigint; metadata: Record<string, unknown>; checkpoint: CheckpointEnvelope }): Promise<void> { await this.db.transaction(async (tx) => { const updated = await tx.query(`UPDATE agent_runs SET metadata=$2::jsonb, version=version+1 WHERE run_id=$1 AND state='RUNNING' AND fencing_token=$3::bigint`, [input.runId, json(input.metadata), input.fencingToken.toString()]); if (updated.rowCount !== 1) throw new Error(`Fenced run progress write rejected for ${input.runId}`); const checkpoint = input.checkpoint; const inserted = await tx.query(`INSERT INTO checkpoints (checkpoint_id,run_id,turn_id,sequence,fencing_token,adapter,adapter_version,schema_version,payload) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)`, [checkpoint.checkpointId, checkpoint.runId, checkpoint.turnId ?? null, checkpoint.sequence.toString(), checkpoint.fencingToken.toString(), checkpoint.adapter, checkpoint.adapterVersion, checkpoint.schemaVersion, checkpoint.payload]); if (inserted.rowCount !== 1) throw new Error(`Checkpoint persistence rejected for ${checkpoint.runId}`); }); }
  async getLatestCheckpoint(runId: string): Promise<CheckpointEnvelope | null> { const result = await this.db.query<Record<string, unknown>>('SELECT * FROM checkpoints WHERE run_id=$1 ORDER BY sequence DESC LIMIT 1',[runId]); const row=result.rows[0]; if(!row)return null; const payload=row.payload instanceof Uint8Array?row.payload:new Uint8Array(row.payload as ArrayBuffer); return { checkpointId:String(row.checkpoint_id),runId:String(row.run_id),turnId:row.turn_id?String(row.turn_id):undefined,sequence:asBigInt(row.sequence),fencingToken:asBigInt(row.fencing_token),createdAt:new Date(String(row.created_at)).toISOString(),adapter:String(row.adapter),adapterVersion:String(row.adapter_version),schemaVersion:Number(row.schema_version),payload }; }
