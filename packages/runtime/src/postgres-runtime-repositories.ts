...
export class PostgresRuntimeRepositories implements DurableRepositories {
  ...
  async appendEventAndOutbox(input: { runId: string; type: string; payload: unknown; topic: string; fencingToken?: bigint }): Promise<RuntimeEventView> {
    return this.db.transaction(async (tx) => { const event = await this.appendEventInTx(tx, input); await tx.query(`INSERT INTO outbox_events (outbox_id,event_id,topic,payload) VALUES ($1,$2,$3,$4::jsonb)`, [`outbox:${event.eventId}`, event.eventId, input.topic, json(event.payload)]); return event; });
  }
