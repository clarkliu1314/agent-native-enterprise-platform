...
import type { DurableRepositories, IdempotencyRecord, OutboxRecord, RunClaim } from './repositories';
...
function toRunView(row: Record<string, unknown>): RunView {
  return {
    ...
    state: row.state as RunView['state'],
    input: row.input,
    metadata: (row.metadata ?? {}) as Record<string, unknown>,
    fencingToken: asBigInt(row.fencing_token),
    attempt: Number(row.attempt),
    createdAt: new Date(String(row.created_at)).toISOString(),
    startedAt: asOptionalIso(row.started_at),
    heartbeatAt: asOptionalIso(row.heartbeat_at),
    finishedAt: asOptionalIso(row.finished_at),
  };
}
...
export class PostgresRuntimeRepositories implements DurableRepositories {
  ...
  async getRun(runId: string): Promise<RunView | null> {
    const result = await this.db.query<Record<string, unknown>>('SELECT * FROM agent_runs WHERE run_id = $1', [runId]);
    return result.rows[0] ? toRunView(result.rows[0]) : null;
  }

  async getIdempotency(key: string): Promise<IdempotencyRecord | null> {
    const result = await this.db.query<Record<string, unknown>>(
      'SELECT idempotency_key, command_hash, run_id, response FROM idempotency_keys WHERE idempotency_key = $1',
      [key],
    );
    const row = result.rows[0];
    if (!row) return null;
    return { key: String(row.idempotency_key), commandHash: String(row.command_hash), runId: row.run_id ? String(row.run_id) : undefined, response: row.response };
  }

  async insertIdempotency(record: IdempotencyRecord): Promise<void> {
    await this.db.query(
      `INSERT INTO idempotency_keys (idempotency_key, command_hash, run_id, response)
       VALUES ($1, $2, $3, $4::jsonb)`,
      [record.key, record.commandHash, record.runId ?? null, json(record.response)],
    );
  }

  async claimRun(runId: string, owner: string, leaseMs: number): Promise<RunClaim | null> {
    const result = await this.db.query<Record<string, unknown>>(
      `UPDATE agent_runs
       SET state = 'RUNNING', lease_owner = $2, lease_expires_at = now() + ($3 * interval '1 millisecond'),
    ...
    const run = toRunView(result.rows[0]);
    return { run, fencingToken: run.fencingToken };
  }

  async renewLease(runId: string, owner: string, fencingToken: bigint, leaseMs: number): Promise<boolean> {
    const result = await this.db.query(
      `UPDATE agent_runs
       SET lease_expires_at = now() + ($4 * interval '1 millisecond'), heartbeat_at = now()
       WHERE run_id = $1 AND lease_owner = $2 AND fencing_token = $3 AND state = 'RUNNING'`,
      [runId, owner, fencingToken.toString(), leaseMs],
    );
    ...
    const result = await this.db.query<Record<string, unknown>>(
      `UPDATE agent_runs
       SET state = $2,
           ...
           metadata = CASE WHEN $5::text IS NULL THEN metadata ELSE jsonb_set(metadata, '{lastError}', to_jsonb($5::text), true) END
       WHERE run_id = $1 AND state = $3
         AND ($4::bigint IS NULL OR fencing_token = $4::bigint)
         AND ($6::text IS NULL OR lease_owner = $6::text)
       RETURNING *`,
      [input.runId, input.to, input.from, input.fencingToken?.toString() ?? null, input.error ?? null, input.owner ?? null],
    );
  ...
  async appendEvent(input: { runId: string; type: string; payload: unknown; fencingToken?: bigint }): Promise<RuntimeEventView> {
    return this.db.transaction(async (tx) => {
      ...
      const event = await tx.query<Record<string, unknown>>(
        `INSERT INTO agent_events (event_id, run_id, sequence, type, payload)
         SELECT $1, run_id, $3, $4, $5::jsonb FROM agent_runs
         WHERE run_id = $2 AND ($6::bigint IS NULL OR fencing_token = $6::bigint)
         RETURNING *`,
        [eventId, input.runId, sequence.toString(), input.type, json(input.payload), input.fencingToken?.toString() ?? null],
      );
  ...
  async listEvents(runId: string, afterSequence?: bigint): Promise<RuntimeEventView[]> {
    const result = await this.db.query<Record<string, unknown>>(
      `SELECT * FROM agent_events WHERE run_id = $1 AND sequence > COALESCE($2::bigint, 0) ORDER BY sequence ASC`,
      [runId, afterSequence?.toString() ?? null],
    );
    return result.rows.map(toEventView);
  }

  async createOutbox(input: { eventId: string; topic: string; payload: unknown }): Promise<void> {
    await this.db.query(
      `INSERT INTO outbox_events (outbox_id, event_id, topic, payload) VALUES ($1, $2, $3, $4::jsonb)`,
      [`outbox:${input.eventId}`, input.eventId, input.topic, json(input.payload)],
    );
  }

  async saveCheckpoint(checkpoint: CheckpointEnvelope): Promise<void> {
    await this.db.query(
      `INSERT INTO checkpoints (checkpoint_id, run_id, turn_id, sequence, fencing_token, adapter, adapter_version, schema_version, payload)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)`,
      [checkpoint.checkpointId, checkpoint.runId, checkpoint.turnId ?? null, checkpoint.sequence.toString(), checkpoint.fencingToken.toString(), checkpoint.adapter, checkpoint.adapterVersion, checkpoint.schemaVersion, checkpoint.payload],
    );
  }

  async getLatestCheckpoint(runId: string): Promise<CheckpointEnvelope | null> {
    const result = await this.db.query<Record<string, unknown>>(
      'SELECT * FROM checkpoints WHERE run_id = $1 ORDER BY sequence DESC LIMIT 1',
      [runId],
    );
    const row = result.rows[0];
    if (!row) return null;
  ...
  async findExpiredRuns(limit: number): Promise<RunView[]> {
    const result = await this.db.query<Record<string, unknown>>(
      `SELECT * FROM agent_runs WHERE state = 'RUNNING' AND lease_expires_at < now() ORDER BY lease_expires_at ASC LIMIT $1`,
      [limit],
    );
    return result.rows.map(toRunView);
  }

  async reclaimExpiredRun(runId: string, owner: string, leaseMs: number): Promise<RunClaim | null> {
    const result = await this.db.query<Record<string, unknown>>(
      `UPDATE agent_runs
       SET lease_owner = $2, lease_expires_at = now() + ($3 * interval '1 millisecond'),
           fencing_token = fencing_token + 1, attempt = attempt + 1, heartbeat_at = now()
  ...
  async saveIdempotencyResponse(key: string, response: unknown): Promise<void> {
    await this.db.query('UPDATE idempotency_keys SET response = $2::jsonb WHERE idempotency_key = $1', [key, json(response)]);
  }
}
