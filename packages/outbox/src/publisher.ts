...
export interface OutboxRepository {
  ...
  /** Mark a message published only after the broker confirms acceptance and the lease still belongs to this worker. */
  markPublished(eventId: string, workerId: string): Promise<void>;
  /** Release a failed claim so the message can be retried with repository-managed backoff/terminal policy. */
