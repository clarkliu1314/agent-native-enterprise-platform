...
const opportunity: InvestmentOpportunity = {
  opportunityId: 'opp-1', tenantId: 'tenant-1', companyId: 'company-1', companyName: 'Acme', stage: 'SCREENING', status: 'ACTIVE', ownerId: 'owner-1', createdAt: '2026-09-12T00:00:00.000Z', updatedAt: '2026-09-12T00:00:00.000Z', version: 2,
};
const decision: InvestmentDecision = {
  decisionId: 'decision-1', opportunityId: 'opp-1', tenantId: 'tenant-1', decisionCycle: 1, recommendation: 'APPROVE', rationale: 'Strong fundamentals', createdAt: '2026-09-12T00:00:00.000Z', idempotencyKey: 'investment-decision:opp-1:1',
};
function sqlClient(responses: Array<{ rows: Record<string, unknown>[]; rowCount: number }>) {
  const calls: Array<{ sql: string; params: readonly unknown[] }> = [];
  return { calls, query: async <T = Record<string, unknown>>(sql: string, params: readonly unknown[] = []) => { calls.push({ sql, params }); return (responses.shift() ?? { rows: [], rowCount: 0 }) as { rows: T[]; rowCount: number }; } };
}
describe('investment PostgreSQL persistence contract', () => {
  it('loads an opportunity from PostgreSQL', async () => {
    const db = sqlClient([{ rows: [{ opportunity_id: opportunity.opportunityId, tenant_id: opportunity.tenantId, company_id: opportunity.companyId, company_name: opportunity.companyName, stage: opportunity.stage, status: opportunity.status, owner_id: opportunity.ownerId, created_at: opportunity.createdAt, updated_at: opportunity.updatedAt, version: opportunity.version }], rowCount: 1 }]);
  ...
  it('reads a decision by tenant-scoped idempotency key', async () => {
    const db = sqlClient([{ rows: [{ decision_id: decision.decisionId, opportunity_id: decision.opportunityId, tenant_id: decision.tenantId, decision_cycle: decision.decisionCycle, recommendation: decision.recommendation, rationale: decision.rationale, created_at: decision.createdAt, idempotency_key: decision.idempotencyKey }], rowCount: 1 }]);
