...
import { Pool } from 'pg';
import { InvestmentApplicationService, PostgresInvestmentUnitOfWork, type InvestmentDatabase, type InvestmentSqlClient } from './application-service';
...
describeIfDatabase('investment transactional command boundary', () => {
  ...
  const database: InvestmentDatabase = {
    async transaction<T>(work: (tx: InvestmentSqlClient) => Promise<T>): Promise<T> {
