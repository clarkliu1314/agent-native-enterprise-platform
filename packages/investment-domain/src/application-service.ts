...
function now(commandNow?: string): string {
  return commandNow ?? new Date().toISOString();
}
...
function decisionConflict(existing: InvestmentDecision, command: ApproveInvestmentCommand | RejectInvestmentCommand): Error | null {
  if (
    existing.opportunityId !== command.opportunityId ||
    existing.tenantId !== command.tenantId ||
    existing.decisionCycle !== command.decisionCycle ||
    existing.recommendation !== command.recommendation ||
...
export class InvestmentApplicationService {
  ...
  approve(command: ApproveInvestmentCommand): Promise<InvestmentDecision> {
    return this.decide(command, 'APPROVE');
  }

  reject(command: RejectInvestmentCommand): Promise<InvestmentDecision> {
    return this.decide(command, 'REJECT');
  }

  private async decide(
    command: ApproveInvestmentCommand | RejectInvestmentCommand,
    ...
    return this.uow.transaction(async ({ opportunities, decisions, events }) => {
      ...
      const decision = createInvestmentDecision({
        ...
        opportunityId: command.opportunityId,
        tenantId: command.tenantId,
        decisionCycle: command.decisionCycle,
        recommendation,
        rationale: command.rationale,
        createdAt: now(command.now),
      });
