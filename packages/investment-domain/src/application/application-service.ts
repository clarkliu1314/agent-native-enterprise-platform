...
import type { AuditRecord } from '@agent-native/runtime';
import type { InvestmentUnitOfWork } from './unit-of-work';
...
function investmentAudit(input: { auditId: string; tenantId: string; actorId: string; action: string; resourceType: string; resourceId: string; occurredAt: string; requestId?: string; traceId?: string; metadata: Record<string, string | number | boolean | null> }): AuditRecord {
  return { auditId: input.auditId, tenantId: input.tenantId, occurredAt: input.occurredAt, actorId: input.actorId, actorType: 'USER', action: input.action, resourceType: input.resourceType, resourceId: input.resourceId, outcome: 'SUCCEEDED', reasonClass: 'NONE', correlation: { requestId: input.requestId ?? input.auditId, traceId: input.traceId ?? input.auditId, ...(input.resourceType === 'INVESTMENT_OPPORTUNITY' ? { workflowId: undefined } : {}) }, metadata: input.metadata };
}
...
export class InvestmentApplicationService {
  ...
  private async decide(command: ApproveInvestmentCommand | RejectInvestmentCommand, recommendation: 'APPROVE' | 'REJECT'): Promise<InvestmentDecision> {
    ...
    return this.uow.transaction(async ({ opportunities, decisions, events, audit, tx }) => {
      ...
      const event: InvestmentDomainEvent = recommendation === 'APPROVE' ? { type: 'InvestmentApproved', eventId: randomUUID(), tenantId: command.tenantId, opportunityId: command.opportunityId, actorId: command.actorId, idempotencyKey: command.idempotencyKey, occurredAt: decision.createdAt, decisionId: persisted.decisionId } : { type: 'InvestmentRejected', eventId: randomUUID(), tenantId: command.tenantId, opportunityId: command.opportunityId, actorId: command.actorId, idempotencyKey: command.idempotencyKey, occurredAt: decision.createdAt, decisionId: persisted.decisionId };
